...
C=''
B=True
import base64 as At,json as K,os as A,re as I,secrets as Au,shutil as J,struct as A6,subprocess as AN,sys as q,tempfile as BX,threading as d,time,urllib.error,urllib.parse,urllib.request,zipfile as Av
from http.server import BaseHTTPRequestHandler as BY,ThreadingHTTPServer as BZ
import webview as Aw
Ba='FiveM Pack Manager'
A7='2.1.0'
r=f"FiveMPackManager/{A7}"
Ax='ImSerial/fivem-pack-manager'
def Bb():
	if AB(q,'frozen',H):C=A.path.join(A.environ.get(Af,A.path.dirname(q.executable)),'FiveMPackManager');A.makedirs(C,exist_ok=B);return C
...
Bc={i:'https://uxqt.site/packs-096759e8/packs.json',v:'glt7ExuP7EBzBc56fUzoAmHy618FWBhT'}
def Bd():
	B=g(Bc);C=[A.path.dirname(A.path.abspath(__file__))]
	...
	for F in C:
		...
		if A.path.exists(E):
			try:
				...
				break
			except(O,K.JSONDecodeError):pass
	return B
Be=Bd()
AQ=AE,'.jpg','.jpeg','.webp','.gif'
Az={AE:'image/png','.jpg':BO,'.jpeg':BO,'.webp':'image/webp','.gif':'image/gif'}
def A_(path,data):
	C=path+'.tmp'
...
def Bf():
	...
	G=A.environ.get(Af,C);D.append(A.path.join(G,Ah,BP))
	for B in D:
		if B and A.path.isdir(B)and(A.path.exists(A.path.join(B,BQ))or A.path.isdir(A.path.join(B,AF))):return B
def Bg(fivem=D):
	I=fivem;M=A8();E=[M.get('gta_path')];J=[I]if I else[];J.append(A.path.join(A.environ.get(Af,C),Ah,BP))
	for K in J:
		G=A.path.join(K,BQ)if K else D
		if G and A.path.exists(G):
			try:
				with Z(G,'r',encoding=U,errors=Ai)as N:
					for L in N:
	...
	for F in E:
		if F and A.path.isdir(F)and A.path.exists(A.path.join(F,BR)):return F
def B0():
	if A.path.exists(AO):
		try:
...
def Y(base,rel):
	...
	return B
Bj=I.compile('[<>:"/\\\\|?*\\x00-\\x1f]')
def A9(name):
	D=name;B=(D or C).strip().strip('. ')
	if not B or Bj.search(B)or B in(j,'..')or A.path.isabs(D or C):raise Q(f"Nom de pack invalide : {D!r}")
	return B
def B4(path):
	try:return t(A.lstat(path).st_file_attributes&1024)
	except(O,AttributeError):return A.path.islink(path)
def AS():
	try:
		...
		for A in D.splitlines():
			...
			if C.startswith((L,'gta5')):return B
		return H
	except G:return H
...
Bk=I.compile('^(enb[\\w .()-]*\\.(ini|dll|asi|fx|fxh|dds|bmp|cfg)|d3d(9|10|11|12)\\.dll|d3dcompiler[\\w.]*\\.dll|dxgi\\.dll)$',I.I)
Bl={'.dll',AH,Aj,'.fx','.fxh','.cfg','.json','.yml','.xml'}
def Bm(gta_base):
	B=gta_base;C={}
	...
	return C
def Bn(src,pack_path,rpf_index,log):
...
AW={L,'five m','five-m','fivem.app','fivem app','fivem files','five m files','fivem folder'}
AX={'reshade-shaders','reshade-presets'}
def Bo(pack_path,log,gta_base=D):
	D=pack_path;I=log;B=[];T=Bm(gta_base);F={}
	def G(key,n=1):F[key]=F.get(key,0)+n
	def O(src):B.append((src,L,A.path.join(A2,Bn(src,D,T,I))));G('rpf vers mods')
	...
	def Q(dirpath,in_fivem=H,depth=0):
		...
		for F in K:
			...
			if A.path.isdir(C):
				if D in AT or D in AX:N=E(B);AV(B,C,L,D);G(f"{D} vers FiveM",E(B)-N)
				elif D in B5:P(C,B6(F))
				elif D in AU:
					if J:N=E(B);AV(B,C,L,D);G(f"{D} vers FiveM",E(B)-N)
					else:P(C,B6(F),prefix=D)
				else:Q(C,J,S+1)
			elif not D.startswith(j):
				M=A.path.splitext(D)[1]
				if M==AI:O(C)
				elif X and Bk.match(F):B.append((C,R,F));G('ENB vers GTA V')
				elif M==AH:B.append((C,L,A.path.join(AG,F)));G(V)
				elif M==Aj and A.path.splitext(D)[0]in Y:B.append((C,L,A.path.join(AG,F)));G(V)
				elif W and M in Bl:B.append((C,L,F));G('racine FiveM')
	Q(D)
	if not B:I("Structure standard non détectée — copie de l'archive telle quelle.");AV(B,D,L,C)
	B=[(E,D,B)for(E,D,B)in B if not(D==L and A.path.dirname(B)==C and A.path.splitext(B)[0].lower()==a)];J,K=set(),[]
	for(U,M,N)in B:
		S=M,N.lower()
		if S not in J:J.add(S);K.append((U,M,N))
...
def z(e):return(L,e)if c(e,h)else(e[0],e[1])
def AY(target,rel):return f"{target}|{rel}"
def Bp(bases,backup_root,manifest,log):
...
n={L:Ah,R:'GTA V'}
Bq={L:{AF},R:{A3,Ak,'redistributables','installers','dlc','_commonredist',A2}}
def B7(plan):
	C={}
	for(G,D,F)in plan:
	...
	return C
def Br(pack_name,bases,state,log,progress):
	...
	try:
		for((H,f),U)in B7(I).items():
			N=S.get(H)
			if H!=L or not N or not A.path.isdir(N):continue
			g=A1((A for A in A.listdir(N)if A.lower()==f),D)
			if g and g!=U:
				try:A.rename(A.path.join(N,g),A.path.join(N,U));K(f"Dossier {g} renommé en {U}.")
				except O:pass
		for((H,f),U)in B7(I).items():
			N=S.get(H)
			if not N or f in Bq.get(H,set()):continue
			t=Y(N,U)
		...
		for(l,(A5,H,V))in AD(I):
			...
			progress(l+1,E(I))
	except G as o:K(f"Erreur pendant l'installation : {o}",F);Bp(S,j,a,K);raise P(f"Installation échouée ({o}) — tout a été annulé.")from o
	e[W][X]=a;B1(e);u=sum(1 for A in a[p]if z(A)[0]==R);A6=f" (dont {u} dans GTA V)"if u else C;K(f"« {X} » chargé : {E(I)} fichiers copiés{A6}, {k} originaux sauvegardés.",M)
def Bs(pack_name,bases,state,log,progress):
	d=bases;V=state;N=pack_name;G=log;R=V[W].get(N)
	if not R:raise Q("Ce pack n'est pas chargé.")
	if AS():raise P('FiveM ou GTA V est ouvert — ferme-les avant de décharger.')
	S=A.path.join(Ay,N);I=R[p];e=set();G(f"Désinstallation de « {N} » — {E(I)} fichiers...");U=0;X=E(I)<=60;i=max(1,E(I)//10)
	for(Z,f)in AD(I):
		C,H=z(f);L=d.get(C)
		if not L:G(f"Cible {n.get(C,C)} introuvable — {H} laissé en place.",F);continue
		try:D=Y(L,H)
	...
	del V[W][N];B1(V);G(f"« {N} » déchargé : {E(I)} fichiers retirés, {U} originaux restaurés.",M)
class AZ(G):0
AA=D
def Bt(fn):global AA;AA=fn
def Aa():
...
def Ac(url,out_path,log,progress,headers=D,make_transform=D,align=1,check_space=B,quiet=H):
	...
	while B:
		...
		try:
			...
			H-=H%align;d=Bv*L;O(f"Coupure réseau ({N}) — reprise dans {d}s à {H/1048576:.0f} Mo (essai {L}/{Ab}).",F);time.sleep(d)
def Ad(url,key):
	A=url
	if not key:return A
...
def By(cfg):
	...
	for A in H:
		if not c(A,g)or not A.get(N):continue
		try:
			A9(A[N])
...
def Bz():
	D=urllib.request.Request(f"https://api.github.com/repos/{Ax}/releases/latest",headers={k:r,BU:'application/vnd.github+json'})
...
def BB(url):
	...
	return Aq,A
BC='Mozilla/5.0'
B_=I.compile('data-id="([\\w-]{20,})"')
C0=I.compile('<title>([^<]*)</title>')
def BD(url,rng=D):
	A={k:BC}
	if rng:A['Range']=rng
...
def C1(html,fallback):
	B=fallback;D=C0.search(html)
	if not D:return B
	A=D.group(1).replace('\xa0',' ');A=I.sub('\\s*[–—-]\\s*Google\\s+Drive\\s*$',C,A).strip();return A or B
def C2(html,self_id):
	B,C=[],{self_id}
	for A in B_.finditer(html):
		if A.group(1)not in C:C.add(A.group(1));B.append(A.group(1))
	return B
def C3(fid):
	for K in range(2):
		try:
			with BD(BE(fid),'bytes=0-0')as A:E=A.headers.get('Content-Disposition',C);L=A.headers.get_content_type();F=A.headers.get(BT,C)
	...
	return D,D,0
def C4(html):return'application/vnd.google-apps.folder'in html or'data-id="'in html
def Ae(seg):A=seg;A=I.sub('[<>:"/\\\\|?*]','_',A).strip(' .');return A or'_'
def C5(folder_id,log):
	B=folder_id;E=[]
	...
	def I(fid,html,prefix,depth):
		J=depth;C=prefix
		if J>8:return
		for B in C2(html,fid):
			L,E,F=C3(B)
			if L:D(B,E,F,C);continue
			try:H=BF(B)
			except G:D(B,E,F,C);continue
			if not C4(H):D(B,E,F,C);continue
			K=Ae(C1(H,B));I(B,H,A.path.join(C,K)if C else K,J+1)
	log('Lecture du dossier Google Drive...');I(B,BF(B),C,0);return E
def C6(folder_id,dest,log,progress):
	I=dest;G=log;C=C5(folder_id,G)
...
def C7(url,log):
	...
	def B(u,data=D,headers=D):
		A=data;B={k:r,BU:AL};B.update(headers or{})
	...
	try:O=urllib.request.urlopen(urllib.request.Request('https://gofile.io/dist/js/global.js',headers={k:r}),timeout=30).read().decode();Q=I.search('wt\\s*[:=]\\s*["\\\']([\\w-]+)["\\\']',O).group(1)
	...
	if A.get(Am)!=M:raise P(f"Gofile a refusé le lien ({A.get(Am)}).")
	...
	H=max(F,key=lambda c:c.get(AM,0));return H['link'],{'Cookie':f"accountToken={C}"},H.get(N)
def BG(s):s=s.replace('-','+').replace('_',X);return At.b64decode(s+'='*(-E(s)%4))
def C8(url,out_path,log,progress):
	L='g';J=b'\x00'
...
def BH(pack,cfg,log,progress):
	V=progress;I=pack;H=log;W=A9(I[N]);P=Y(T,W);F=P+'.part';A.makedirs(T,exist_ok=B);j,L=BX.mkstemp(suffix='.pack',dir=T);A.close(j);O=D
	try:
		H(f"Téléchargement de « {I[N]} »...")
		if AS():H("Note : FiveM est ouvert — le téléchargement passe, mais ferme-le avant l'installation.")
		O,Q=BB(I[l]);K=I.get(Ao)
		if A.path.isdir(F):J.rmtree(F,ignore_errors=B)
		if O==Ap:C6(Q,F,H,V);BL(F,H)
		elif O=='mega':K=C8(Q,L,H,V)or K
		else:
			if O==BV:H('Résolution du lien Gofile...');X,e,k=C7(Q,H);K=K or k
			else:X,e=Q,{}
		...
		if O!=Ap:
			...
			if not CC(F):BL(F,H)
		if I.get(b):
			with Z(A.path.join(F,BS),'w',encoding=U)as d:d.write(h(I[b]))
		if I.get(S):
			try:
				with B9(I[S],D)as n:
		...
		A.replace(F,P);H(f"« {W} » téléchargé et extrait.",M)
...
def BL(dest,log):
	...
	for S in range(3):
		D=[]
		for(P,T,Q)in A.walk(dest):D+=[A.path.join(P,B)for B in Q if A.path.splitext(B)[1].lower()in C9 or o.search(B)or A0.search(B)]
...
class CE:
	def __init__(A):A.state=B0();A.cfg=A8();A.fivem=Bf();A.gta=Bg(A.fivem);A.remote_packs=[];A.custom_packs=u(A.cfg.get('custom_packs',[]));A.background=A.cfg.get(As);A.busy=H;A._cancel=d.Event();Bt(A._cancel.is_set);A._lock=d.Lock();A._buf_lock=d.Lock();A._logs=[];A._prog=0,0;A._dirty=H
	...
	def _all_remote(C):
		D={A[N]:g(A)for A in C.remote_packs}
		for E in C.custom_packs:A=g(E);A[A5]=B;D[A[N]]=A
		return u(D.values())
	...
	def get_state(A):
		Q='remote';P='image_link';O='nfiles';J=[];K={A[N]:A for A in A._all_remote()}
		for I in AR():F=K.pop(I,D);M=B3(I);J.append({N:I,AM:Bh(I),b:M,W:I in A.state[W],O:E(A.state[W].get(I,{}).get(p,[])),S:(F or{}).get(S)or Bi(I),P:(F or{}).get(S),l:(F or{}).get(l),a:(F or{}).get(a),Q:H,A5:t(F and F.get(A5)),A3:t(F and F.get(b)and h(F[b])!=(M or C))})
		for G in K.values():J.append({N:G[N],AM:G.get(AM,C),b:G.get(b),W:H,O:0,S:G.get(S),P:G.get(S),l:G.get(l),a:G.get(a),Q:B,A5:t(G.get(A5)),A3:H})
		return{L:A.fivem,R:A.gta,Ag:J,As:A.background_url(),i:A.cfg.get(i,C),v:A.cfg.get(v,C),'background_setting':A.background or C,'busy':A.busy,b:A7}
	...
	def add_custom_pack(A,name,url,image,preview=C,old_name=C):
		...
		try:D=A9(D);BB(H)
		except G as O:A._log(f"Refusé : {O}",F);return
		...
		P={D,E}-{C};A.custom_packs=[A for A in A.custom_packs if A[N]not in P];L={N:D,l:H}
		if K:L[S]=K
		if I:L[a]=I
		...
		A._log(f"Pack « {D} » {"modifié"if E else"ajouté"}.",M);A._refresh_ui()
	def preview(C,name):
		E=A1((A for A in C._all_remote()if A[N]==name),D);B=(E or{}).get(a)
	...
	def remove_custom_pack(B,name):
		...
		B.custom_packs=[A for A in B.custom_packs if A[N]!=C];f(custom_packs=B.custom_packs)
		try:E=Y(T,C)
		except Q:E=D
		if E and A.path.isdir(E):
			try:J.rmtree(E);B._log(f"Pack « {C} » retiré (fichiers téléchargés supprimés).",M)
			except O as G:B._log(f"Pack « {C} » retiré, mais cache non supprimé : {G}",F)
		else:B._log(f"Pack « {C} » retiré.",M)
		B._refresh_ui()
	def choose_background(A):return CD()or C
	def _set_background(C,bg):
		B=bg;B=B.strip()
		if not B:C.background=D;f(background=D);C._log('Image de fond retirée.',M)
		elif B.startswith((w,x)):C.background=B;f(background=B);C._log('Image de fond (lien) enregistrée.',M)
		elif A.path.isfile(B):
			for H in('background.png','background.jpg','background.jpeg','background.webp'):
				try:A.remove(A.path.join(e,H))
				except O:pass
			E=A.path.splitext(B)[1].lower();E=E if E in AQ else AE;G=As+E;J.copy2(B,A.path.join(e,G));C.background=G;f(background=G);C._log('Image de fond enregistrée.',M)
	...
	def save_settings(B,url,key,fivem,gta,bg):
		...
		if E:
			if A.path.isdir(E):B.fivem=E;f(fivem_path=E);B._log(f"Dossier FiveM : {E}",M)
			else:B._log(f"Dossier introuvable : {E}",F)
		D=D.strip()
		if D:
			if A.path.isdir(D)and A.path.exists(A.path.join(D,BR)):B.gta=D;f(gta_path=D);B._log(f"Dossier GTA V : {D}",M)
	...
	def check_update(C):
		def A():
			try:
				A=Bz()
				if A and BA(A[0])>BA(A7):C._log(f"Nouvelle version disponible : {A[0]} (tu as la {A7}) — {A[1]}",M)
			except G:pass
		d.Thread(target=A,daemon=B).start()
	def fetch_remote(A):
		if not A.cfg.get(i):A._log("Pas d'URL de serveur configurée (voir Options).",F);return
		def C():
			try:A._log('Connexion au serveur de packs...');A.remote_packs=By(A.cfg);A._log(f"{E(A.remote_packs)} pack(s) disponibles en ligne.",M)
...
def CH(api):
	...
	class O(BY):
		...
		def do_POST(A):
			...
			if not A._host_ok()or B not in CG or not Au.compare_digest(A.headers.get(BW)or C,I):A._send(403,M,F);return
			try:
	...
	J=BZ((L,0),O);d.Thread(target=J.serve_forever,daemon=B).start();return J,f"http://127.0.0.1:{J.server_address[1]}/",I
def CI():
	A=AC(B0().get(W,{}))
	try:print('\n'.join(A))
	except G:pass
...
def CJ():
	if'--check-loaded'in q.argv:CI()
	H=CE();I,E,J=CH(H);K=[J];D=Aw.create_window(Ba,url=E,width=980,height=720,min_size=(700,520),background_color='#12121a')
	if A.environ.get('PM_SELFTEST'):
		import time as F
		def C(*A):C=' '.join(h(A)for A in A);print(C.encode('ascii',Ai).decode(),flush=B)
		def L():
			F.sleep(4)
			try:import urllib.request as B;H=B.Request(E+'api/poll',data=b'[]',method='POST');H.add_header(BW,K[0]);I=B.urlopen(H,timeout=5).read()[:80];C('SELFTEST urllib POST:',I)
