...
C=len
B=True
import base64 as Ap,json as L,os as A,re as H,secrets,shutil as I,struct as AK,subprocess as Aq,sys,tempfile as BI,threading as i,urllib.parse,urllib.request,zipfile as Ar
from http.server import BaseHTTPRequestHandler as BJ,ThreadingHTTPServer as BK
import webview as As
BL='FiveM Pack Manager'
m='FiveMPackManager/2.0'
def BM():
	if Aa(sys,'frozen',M):C=A.path.join(A.environ.get(Ac,A.path.dirname(sys.executable)),'FiveMPackManager');A.makedirs(C,exist_ok=B);return C
	return A.path.dirname(A.path.abspath(__file__))
b=BM()
O=A.path.join(b,Ad)
At=A.path.join(b,'_backups')
AL=A.path.join(b,'state.json')
AM=A.path.join(b,'config.json')
BN={e:'https://uxqt.site/packs-096759e8/packs.json',q:'glt7ExuP7EBzBc56fUzoAmHy618FWBhT'}
def BO():
	B=u(BN);C=[A.path.dirname(A.path.abspath(__file__))]
	if Aa(sys,'_MEIPASS',E):C.insert(0,sys._MEIPASS)
	for F in C:
		D=A.path.join(F,'embedded_config.json')
		if A.path.exists(D):
			try:
				with P(D,x,encoding=U)as G:B.update(L.load(G))
				break
			except(T,L.JSONDecodeError):pass
	return B
BP=BO()
AN=AA,'.jpg','.jpeg','.webp','.gif'
Au={AA:'image/png','.jpg':BB,'.jpeg':BB,'.webp':'image/webp','.gif':'image/gif'}
def A4():
	B=u(BP)
	if A.path.exists(AM):
		try:
			with P(AM,x,encoding=U)as C:B.update(L.load(C))
		except(T,L.JSONDecodeError):pass
	return B
def c(**B):
	A=A4();A.update(B)
	with P(AM,'w',encoding=U)as C:L.dump(A,C,indent=2)
def BQ():
	F='fivem_path';C=[];E=A4()
	if E.get(F):C.append(E[F])
	G=A.environ.get(Ac,D);C.append(A.path.join(G,Ae,BC))
	for B in C:
		if B and A.path.isdir(B)and(A.path.exists(A.path.join(B,BD))or A.path.isdir(A.path.join(B,AB))):return B
def BR(fivem=E):
	I=fivem;M=A4();C=[M.get('gta_path')];J=[I]if I else[];J.append(A.path.join(A.environ.get(Ac,D),Ae,BC))
	for K in J:
		G=A.path.join(K,BD)if K else E
		if G and A.path.exists(G):
			try:
				with P(G,x,encoding=U,errors=Af)as N:
					for L in N:
	...
	for F in C:
		if F and A.path.isdir(F)and A.path.exists(A.path.join(F,BE)):return F
def BS():
	if A.path.exists(AL):
		try:
			with P(AL,x,encoding=U)as B:return L.load(B)
		except(T,L.JSONDecodeError):pass
	return{W:{}}
def Av(state):
	with P(AL,'w',encoding=U)as A:L.dump(state,A,indent=2,ensure_ascii=M)
def AO():A.makedirs(O,exist_ok=B);return Ab(B for B in A.listdir(O)if A.path.isdir(A.path.join(O,B))and not B.startswith(r))
def Bz(pack_path):
	B=pack_path
	for(C,H,F)in A.walk(B):
		...
		for D in F:
			E=D.lower()
			if E.startswith(r)or G and A.path.splitext(E)[0]==X:continue
...
def Aw(pack_name):
	...
	for B in AN:
		...
		if A.path.exists(C):
			try:
				with P(C,'rb')as E:F=Ap.b64encode(E.read()).decode('ascii')
				return f"data:{Au[B]};base64,{F}"
			except T:return
def Ax(name):
	B=A.path.join(O,name,BF)
	if A.path.exists(B):
		try:
			with P(B,x,encoding=U)as C:return C.read().strip()
		except T:pass
def d(base,rel):
...
def AP():
	try:
		D=Aq.run(['tasklist','/FO','CSV'],capture_output=B,text=B,creationflags=B6,timeout=10).stdout.lower()
...
def B_(path):
	...
	return B
AQ={AB,y,AC}
Ay={'gtav','gta5','gta v','gta 5','grand theft auto v','grand theft auto 5','grand theft auto v legacy','gta v legacy','gtav legacy','gta 5 legacy','gta5 legacy','singleplayer','single player',R}
AR={'enbseries','enbcache'}
BU=H.compile('^(enb[\\w .()-]*\\.(ini|dll|asi|fx|fxh|dds|bmp|cfg)|d3d(9|10|11|12)\\.dll|d3dcompiler[\\w.]*\\.dll|dxgi\\.dll)$',H.I)
BV={'.dll',AD,Ag,'.fx','.fxh','.cfg','.json','.yml','.xml'}
def BW(gta_base):
	B=gta_base;C={}
	if not B or not A.path.isdir(B):return C
	for(F,E,G)in A.walk(B):
		E[:]=[A for A in E if A.lower()!=y]
		for D in G:
			if D.lower().endswith(AE):H=A.path.relpath(A.path.join(F,D),B);C.setdefault(D.lower(),[]).append(H)
	return C
def BX(src,pack_path,rpf_index,log):
	B=A.path.basename(src);D=A.path.relpath(src,pack_path).split(A.sep);H=[A.lower()for A in D]
	for(F,G)in A8(H[:-1]):
		if G in(z,Ah):return A.path.join(*D[F:])
		if G=='dlcpacks':return A.path.join(z,Ah,*D[F:])
...
AT={J,'five m','five-m','fivem.app','fivem app','fivem files','five m files','fivem folder'}
...
def BY(pack_path,log,gta_base=E):
	...
	def N(src):B.append((src,J,A.path.join(y,BX(src,E,T,F))));G('rpf vers mods')
	def O(gta_dir,label,prefix=D):
		...
		for(I,K,J)in A.walk(E):
			for C in J:
				...
				D=A.path.join(I,C)
				if C.lower().endswith(AE):N(D)
				else:H=A.path.relpath(D,E);B.append((D,R,A.path.join(F,H)if F else H));G(f"{label} vers GTA V")
	def P(dirpath,in_fivem=M):
		T='asi vers plugins';I=dirpath;H=in_fivem;K=Ab(A.listdir(I));Q={B.lower()for B in K if A.path.isdir(A.path.join(I,B))};S=A.path.basename(I).lower();H=H or S in AT;U=S in AT or v(Q&(AQ|AU));V=not H and(v(Q&AR)or any(A.lower().startswith('enb')and A.lower().endswith(Ag)for A in K));W={A.path.splitext(B)[0].lower()for B in K if B.lower().endswith(AD)}
		for F in K:
			E=A.path.join(I,F);D=F.lower()
			if A.path.isdir(E):
				if D in AQ or D in AU:M=C(B);AS(B,E,J,D);G(f"{D} vers FiveM",C(B)-M)
				elif D in Ay:O(E,Az(F))
				elif D in AR:
					if H:M=C(B);AS(B,E,J,D);G(f"{D} vers FiveM",C(B)-M)
					else:O(E,Az(F),prefix=D)
				else:P(E,H)
			elif not D.startswith(r):
				L=A.path.splitext(D)[1]
				if L==AE:N(E)
				elif V and BU.match(F):B.append((E,R,F));G('ENB vers GTA V')
				elif L==AD:B.append((E,J,A.path.join(AC,F)));G(T)
				elif L==Ag and A.path.splitext(D)[0]in W:B.append((E,J,A.path.join(AC,F)));G(T)
				elif U and L in BV:B.append((E,J,F));G('racine FiveM')
	P(E)
	if not B:F("Structure standard non détectée — copie de l'archive telle quelle.");AS(B,E,J,D)
...
def Az(name):A=name;return A if C(A)<=20 else A[:17]+'...'
def s(e):return(J,e)if p(e,f)else(e[0],e[1])
...
def BZ(bases,backup_root,manifest,log):
	...
	for(D,N)in BA(K.get(AF,[])):
		E=M.get(D)
		if not E:continue
		try:
			C=d(E,N);H=A.path.join(J,Ai,D,N)
			if A.path.exists(H):
...
n={J:Ae,R:'GTA V'}
Ba={J:{AB},R:{z,Ah,'redistributables','installers','dlc','_commonredist',y}}
def A_(plan):
	D={}
	for(G,E,F)in plan:
...
def Bb(pack_name,bases,state,log,progress):
	...
	if AP():raise Q('FiveM ou GTA V est ouvert — ferme-les avant de charger un pack.')
	u=A.path.join(O,X);K=BY(u,L,S.get(R))
	...
	v=sum(A.path.getsize(B)for(B,C,D)in K if A.path.exists(B));A5(S[J],v,"l'installation");Y={k:[],A0:{},AF:[]};Z={}
	for(a,x)in c[W].items():
		if a!=X:
			for p in x[k]:Z[s(p)[0]+'|'+s(p)[1].lower()]=a
	L(f"Installation de « {X} » — {C(K)} fichiers...");h=A.path.join(At,X);i=0;q=C(K)<=60;y=max(1,C(K)//10)
	try:
		for((H,e),P)in A_(K).items():
			M=S.get(H)
			if H!=J or not M or not A.path.isdir(M):continue
			f=w((A for A in A.listdir(M)if A.lower()==e),E)
			if f and f!=P:
		...
		for((H,e),P)in A_(K).items():
			...
			r=d(M,P)
			if not A.path.isdir(r):continue
			z=f"{H}|{e}{A.sep}";a=w((B for(A,B)in Z.items()if A.startswith(z)),E)
			if a:L(f"Dossier {P} : contient des fichiers du pack « {a} » — fusion au lieu du remplacement.");continue
			b=A.path.join(h,Ai,H,P);A.makedirs(A.path.dirname(b),exist_ok=B);I.move(r,b);Y[AF].append([H,P]);L(f"Dossier existant mis de côté ({n[H]}) : {P} — remplacé proprement.")
		for(j,(A1,H,U))in A8(K):
			...
			if A.path.exists(g)and l not in Z:
				b=A.path.join(h,H,U);A.makedirs(A.path.dirname(b),exist_ok=B);I.copy2(g,b);Y[A0][AV(H,U)]=B;i+=1
				if q:L(f"Sauvegarde de l'original ({n[H]}) : {U}")
			I.copy2(A1,g);Y[k].append([H,U])
			if q:L(f"Copie ({n[H]}) : {U}")
			elif(j+1)%y==0:L(f"{j+1}/{C(K)} fichiers copiés ({i} originaux sauvegardés)...")
			progress(j+1,C(K))
	except G as m:L(f"Erreur pendant l'installation : {m}",F);BZ(S,h,Y,L);raise Q(f"Installation échouée ({m}) — tout a été annulé.")from m
	c[W][X]=Y;Av(c);t=sum(1 for A in Y[k]if s(A)[0]==R);A2=f" (dont {t} dans GTA V)"if t else D;L(f"« {X} » chargé : {C(K)} fichiers copiés{A2}, {i} originaux sauvegardés.",N)
def Bc(pack_name,bases,state,log,progress):
	...
	if AP():raise Q('FiveM ou GTA V est ouvert — ferme-les avant de décharger.')
	P=A.path.join(At,M);J=O[k];b=set();G(f"Désinstallation de « {M} » — {C(J)} fichiers...");S=0;X=C(J)<=60;g=max(1,C(J)//10)
	for(Y,c)in A8(J):
		D,H=s(c);K=a.get(D)
		...
		e,i=A.path.join(P,D,H),A.path.join(P,H);j=O[A0].get(AV(D,H))or p(c,f)and O[A0].get(H)
		if j:
	...
	del U[W][M];Av(U);G(f"« {M} » déchargé : {C(J)} fichiers retirés, {S} originaux restaurés.",N)
class AW(G):0
A6=E
def Bd(fn):global A6;A6=fn
def A7():
	if A6 is not E and A6():raise AW('Téléchargement annulé.')
def AX(url,key):
	A=url
	if not key:return A
	B='&'if'?'in A else'?';return f"{A}{B}key={urllib.parse.quote(key)}"
def B0(url,key):A=urllib.request.Request(AX(url,key),headers={l:m});return urllib.request.urlopen(A,timeout=30)
def Be(cfg):
	C=cfg.get(e)
	if not C:return[]
	D=cfg.get(q)
	with B0(C,D)as G:B=L.loads(G.read().decode(U))
	E=C.rsplit(Z,1)[0]+Z;F=B.get(Ad,B)if p(B,u)else B
	for A in F:
		if not A.get(g):A[g]=AX(urllib.parse.urljoin(E,A[Aj]),D)
		if A.get(S)and not A[S].startswith((A1,A2,'data:')):A[S]=AX(urllib.parse.urljoin(E,A[S]),D)
	return F
def B1(url):
	D='drive.google.com';A=url.strip();B=A.lower()
	if'mega.nz'in B or'mega.co.nz'in B:return'mega',A
	if'gofile.io'in B:return BG,A
	if D in B and'/folders/'in B:
		C=H.search('/folders/([\\w-]+)',A)
		if C:return AG,C.group(1)
	if D in B:
		C=H.search('/file/d/([\\w-]+)',A)or H.search('[?&]id=([\\w-]+)',A)
		if C:return Ak,f"https://drive.usercontent.google.com/download?id={C.group(1)}&export=download&confirm=t"
...
def B2(fid):return f"https://drive.usercontent.google.com/download?id={fid}&export=download&confirm=t"
def B3(fid):
	with AY(f"https://drive.google.com/drive/folders/{fid}")as A:return A.read().decode(U,Af)
...
def Bk(fid):
	for J in range(2):
		try:
			with AY(B2(fid),'bytes=0-0')as A:C=A.headers.get('Content-Disposition',D);K=A.headers.get_content_type();F=A.headers.get('Content-Range',D)
...
def AZ(seg):A=seg;A=H.sub('[<>:"/\\\\|?*]','_',A).strip(' .');return A or'_'
def Bm(folder_id,log):
	B=folder_id;C=[]
	def E(cid,fname,size,prefix):E=prefix;D=fname;B=cid;F=A.path.join(E,AZ(D or B))if E else AZ(D or B);C.append((F,B,size))
	def I(fid,html,prefix,depth):
		...
		for B in Bj(html,fid):
			...
			K=AZ(Bi(H,B));I(B,H,A.path.join(C,K)if C else K,J+1)
	log('Lecture du dossier Google Drive...');I(B,B3(B),D,0);return C
def Bn(folder_id,dest,log,progress):
	...
	E=sum(A for(B,C,A)in D);G(f"{C(D)} fichiers dans le dossier"+(f" ({E/1048576:.0f} Mo)."if E else r))
	if E:A5(F,E,Al)
	A.makedirs(F,exist_ok=B);J=0;L=max(1,C(D)//20)
	for(H,(M,N,S))in A8(D):
		A7();K=d(F,M);A.makedirs(A.path.dirname(K),exist_ok=B)
		with AY(B2(N))as O,P(K,AH)as R:
			while B:
				A7();I=O.read(262144)
				if not I:break
				R.write(I);J+=C(I)
...
def Bo(url,log):
	M='status';J='data';O=url.rstrip(Z).split(Z)[-1].split('?')[0]
	def B(u,data=E,headers=E):
		A=data;B={l:m,'Accept':AI};B.update(headers or{})
		if A is not E:B[Am]=AI;A=L.dumps(A).encode()
	...
	try:P=urllib.request.urlopen(urllib.request.Request('https://gofile.io/dist/js/global.js',headers={l:m}),timeout=30).read().decode();R=H.search('wt\\s*[:=]\\s*["\\\']([\\w-]+)["\\\']',P).group(1)
	...
	S=A[J];T=S.get('children')or{};F=[A for A in T.values()if A.get('type')==Aj]
	if not F:raise Q('Gofile : aucun fichier dans ce lien (dossier vide ?).')
	I=max(F,key=lambda c:c.get(AJ,0));return I['link'],{'Cookie':f"accountToken={C}"},I.get(K)
def B4(s):s=s.replace('-','+').replace('_',Z);return Ap.b64decode(s+'='*(-C(s)%4))
def Bp(url,out_path,log,progress):
	...
	except B9 as b:raise Q('Support Mega indisponible (module cryptography manquant).')from b
	...
	c,d=I.group(1),I.group(2);A=AK.unpack('>8I',B4(d));U=AK.pack('>4I',A[0]^A[4],A[1]^A[5],A[2]^A[6],A[3]^A[7]);e=AK.pack('>2I',A[4],A[5])+M*8;f=urllib.request.Request('https://g.api.mega.co.nz/cs?id=0',data=L.dumps([{'a':N,N:1,'p':c}]).encode(),headers={Am:AI,l:m});E=L.loads(urllib.request.urlopen(f,timeout=30).read().decode())
	if p(E,Y)or p(E,A9)and p(E[0],Y):raise Q('Mega a refusé le lien (fichier supprimé ou clé invalide).')
	E=E[0];g,F=E[N],Y(E.get('s',0));J='mega_pack'
	try:
		V=R(S.AES(U),T.CBC(M*16)).decryptor();W=V.update(B4(E['at']))+V.finalize()
		if W.startswith(b'MEGA'):J=L.loads(W[4:].split(M)[0].decode())['n']
	except G:pass
	if F:A5(O,Y(F*2.3),Al)
	log(f"Fichier Mega : {J}"+(f" ({F/1048576:.0f} Mo)"if F else D));X=R(S.AES(U),T.CTR(e)).decryptor();Z=0
	with urllib.request.urlopen(urllib.request.Request(g,headers={l:m}),timeout=60)as h,P(out_path,AH)as a:
		while B:
			A7();K=h.read(262144)
			if not K:break
		...
		a.write(X.finalize())
	return J
def B5(pack,cfg,log,progress):
	b=progress;J=log;H=pack;F=A.path.join(O,H[K]);A.makedirs(O,exist_ok=B);p,V=BI.mkstemp(suffix='.pack',dir=O);A.close(p);M=E
	try:
		J(f"Téléchargement de « {H[K]} »...")
		if AP():J("Note : FiveM est ouvert — le téléchargement passe, mais ferme-le avant l'installation.")
		M,W=B1(H[g]);L=H.get(Aj)
		if M==AG:
			if A.path.isdir(F):I.rmtree(F)
			Bn(W,F,J,b);B8(F,J)
		elif M=='mega':L=Bp(W,V,J,b)or L
		else:
			if M==BG:J('Résolution du lien Gofile...');c,i,q=Bo(W,J);L=L or q
			else:c,i=W,{}
			j={l:m};j.update(i)
			with urllib.request.urlopen(urllib.request.Request(c,headers=j),timeout=60)as R:
				...
				T=Y(R.headers.get(An,0))
				if T:A5(O,Y(T*2.3),Al)
		...
		if M!=AG:
			...
			if A.path.isdir(F):I.rmtree(F)
			B7(V,F,J);Z=A.listdir(F)
			if C(Z)==1 and A.path.isdir(A.path.join(F,Z[0]))and Z[0].lower()not in(AB,y,AC):
				e=A.path.join(F,Z[0])
				for n in A.listdir(e):I.move(A.path.join(e,n),A.path.join(F,n))
				A.rmdir(e)
			if not Bs(F):B8(F,J)
		if H.get(a):
			with P(A.path.join(F,BF),'w',encoding=U)as h:h.write(f(H[a]))
		if H.get(S)and not Aw(H[K]):
			try:
				with B0(H[S],E)as R:
					o=A.path.splitext(urllib.parse.urlparse(H[S]).path)[1]or AA
					if o.lower()in AN:
						with P(A.path.join(F,X+o.lower()),AH)as h:h.write(R.read())
			except G:pass
		J(f"« {H[K]} » téléchargé et extrait.",N)
	except AW:
		if M==AG and A.path.isdir(F):I.rmtree(F,ignore_errors=B)
		raise
	finally:
...
t=H.compile('\\.r\\d{2}$',H.I)
j=H.compile('\\.(\\d{3})$')
def Br():J='-o{d}';I='7-Zip';H='-inul';G='-ibck';F='WinRAR';E='UnRAR';D='{d}\\';C='-y';B='x';K=[(E,['C:\\Program Files\\WinRAR\\UnRAR.exe',B,C,h,D]),(E,['C:\\Program Files (x86)\\WinRAR\\UnRAR.exe',B,C,h,D]),(F,['C:\\Program Files\\WinRAR\\WinRAR.exe',B,G,H,C,h,D]),(F,['C:\\Program Files (x86)\\WinRAR\\WinRAR.exe',B,G,H,C,h,D]),(I,['C:\\Program Files\\7-Zip\\7z.exe',B,C,J,h]),(I,['C:\\Program Files (x86)\\7-Zip\\7z.exe',B,C,J,h]),('tar',[A.path.join(A.environ.get('SystemRoot','C:\\Windows'),'System32','tar.exe'),'-xf',h,'-C','{d}'])];return[(C,B)for(C,B)in K if A.path.exists(B[0])]
def B7(archive,dest,log):
	D=archive;C=dest;A.makedirs(C,exist_ok=B)
	if Ar.is_zipfile(D):
		try:
			with Ar.ZipFile(D)as H:
				for I in H.namelist():
	...
	K=[]
	for(L,E)in J:
		log(f"Extraction avec {L}...");E=[A.replace(h,D).replace('{d}',C)for A in E];F=Aq.run(E,capture_output=B,text=B,creationflags=B6)
...
def B8(dest,log):
	...
	for S in range(3):
		C=[]
		for(P,T,Q)in A.walk(dest):C+=[A.path.join(P,B)for B in Q if A.path.splitext(B)[1].lower()in Bq or j.search(B)or t.search(B)]
		C=[A for A in C if A not in M]
		if not C:return
		H=[]
		for B in C:
			E=A.path.basename(B)
			if t.search(E):continue
			J=j.search(E)
			if J and J.group(1)!='001':continue
			K=o.search(E)
			if K and Y(K.group(1))>1:continue
			if K:N=o.sub(D,E)
			elif J:O=j.sub(D,E);N=A.path.splitext(O)[0]or O
			else:N=A.path.splitext(E)[0]
			L(f"Archive dans le pack : {E} — extraction...")
			try:B7(B,A.path.join(A.path.dirname(B),N),L)
			...
			elif J:I=j.sub(D,B).lower();H+=[A for A in C if A!=B and j.search(A)and j.sub(D,A).lower()==I]
			elif E.lower().endswith('.rar'):I=B[:-4].lower();H+=[A for A in C if t.search(A)and t.sub(D,A).lower()==I]
		for B in C:
			if B in H:
				if A.path.exists(B):A.remove(B)
			elif o.search(B)or t.search(B)or j.search(B):M.add(B)
def Bs(dest):
	C=AQ|Ay|AT|AU|AR
	for(F,D,E)in A.walk(dest):
		if any(A.lower()in C for A in D):return B
		if any(A.lower().endswith((AE,AD))for A in E):return B
	return M
def Bt():
	try:
...
class Bu:
	def __init__(A):A.state=BS();A.cfg=A4();A.fivem=BQ();A.gta=BR(A.fivem);A.remote_packs=[];A.custom_packs=A9(A.cfg.get('custom_packs',[]));A.background=A.cfg.get(Ao);A.busy=M;A._cancel=i.Event();Bd(A._cancel.is_set);A._lock=i.Lock();A._buf_lock=i.Lock();A._logs=[];A._prog=0,0;A._dirty=M
	...
	def poll(A):
		...
		return{'logs':B,'prog':A9(A._prog),'busy':A.busy,'dirty':C}
	def _all_remote(C):
		D={A[K]:u(A)for A in C.remote_packs}
		for E in C.custom_packs:A=u(E);A[A3]=B;D[A[K]]=A
		return A9(D.values())
	...
	def get_state(A):
		...
		for H in AO():F=L.pop(H,E);N=Ax(H);I.append({K:H,AJ:BT(H),a:N,W:H in A.state[W],O:C(A.state[W].get(H,{}).get(k,[])),S:Aw(H)or(F or{}).get(S),P:(F or{}).get(S),g:(F or{}).get(g),X:(F or{}).get(X),Q:M,A3:v(F and F.get(A3)),z:v(F and F.get(a)and f(F[a])!=(N or D))})
		for G in L.values():I.append({K:G[K],AJ:G.get(AJ,D),a:G.get(a),W:M,O:0,S:G.get(S),P:G.get(S),g:G.get(g),X:G.get(X),Q:B,A3:v(G.get(A3)),z:M})
		return{J:A.fivem,R:A.gta,Ad:I,Ao:A.background_url(),e:A.cfg.get(e,D),q:A.cfg.get(q,D),'background_setting':A.background or D,'busy':A.busy}
	...
	def add_custom_pack(C,name,url,image,preview=D,old_name=D):
		...
		R={H,E}-{D};C.custom_packs=[A for A in C.custom_packs if A[K]not in R];P={K:H,g:L}
		if M:P[S]=M
		if J:P[X]=J
		C.custom_packs.append(P);c(custom_packs=C.custom_packs)
		if E and E!=H and E in AO():I.rmtree(A.path.join(O,E),ignore_errors=B)
		C._log(f"Pack « {H} » {"modifié"if E else"ajouté"}.",N);C._refresh_ui()
	def preview(C,name):
		D=w((A for A in C._all_remote()if A[K]==name),E);B=(D or{}).get(X)
	...
	def _set_background(C,bg):
		...
		if not B:C.background=E;c(background=E);C._log('Image de fond retirée.',N)
			...
			D=A.path.splitext(B)[1].lower();D=D if D in AN else AA;G=Ao+D;I.copy2(B,A.path.join(b,G));C.background=G;c(background=G);C._log('Image de fond enregistrée.',N)
		else:C._log(f"Image introuvable : {B}",F)
	def save_settings(B,url,key,fivem,gta,bg):
		E=fivem;C=gta;B.cfg[e]=url.strip();B.cfg[q]=key.strip();c(packs_url=B.cfg[e],packs_key=B.cfg[q]);E=E.strip()
		if E:
			if A.path.isdir(E):B.fivem=E;c(fivem_path=E);B._log(f"Dossier FiveM : {E}",N)
			else:B._log(f"Dossier introuvable : {E}",F)
		C=C.strip()
		if C:
			if A.path.isdir(C)and A.path.exists(A.path.join(C,BE)):B.gta=C;c(gta_path=C);B._log(f"Dossier GTA V : {C}",N)
		...
		if B.cfg[e]:B.fetch_remote()
		else:B.remote_packs=[];B._refresh_ui()
	def fetch_remote(A):
		if not A.cfg.get(e):A._log("Pas d'URL de serveur configurée (voir Options).",F);return
		def D():
			...
			A._refresh_ui()
		i.Thread(target=D,daemon=B).start()
	def _run(A,fn):
		def C():
			with A._lock:
		...
		i.Thread(target=C,daemon=B).start()
	def cancel(A):
	...
	def load(A,name):
		...
		def B():
			B=w((A for A in A._all_remote()if A[K]==C),E);F=C in AO();G=B and B.get(a)and f(B[a])!=(Ax(C)or D)
			if B and(not F or G):B5(B,A.cfg,A._log,A._progress)
	...
	def download(A,name):
		B=w((A for A in A._all_remote()if A[K]==name),E)
		if not B:A._log(f"Pack « {name} » introuvable sur le serveur.",F);return
		A._run(lambda:B5(B,A.cfg,A._log,A._progress))
...
def Bx(api):
	...
	class J(BJ):
		...
		def _send(A,code,body,ctype):A.send_response(code);A.send_header(Am,ctype);A.send_header(An,f(C(body)));A.send_header('Cache-Control','no-store');A.end_headers();A.wfile.write(body)
		def do_GET(B):
			if B.path in(Z,'/index.html'):B._send(200,I,'text/html; charset=utf-8')
			elif B.path.startswith('/bg'):
				D=api.background;C=A.path.join(b,D)if D and not D.startswith(Ak)else E
				if C and A.path.exists(C):
					F=A.path.splitext(C)[1].lower()
					with P(C,'rb')as G:B._send(200,G.read(),Au.get(F,'application/octet-stream'))
				else:B._send(404,b'no background',H)
			else:B._send(404,b'not found',H)
		def do_POST(A):
			B=A.path.removeprefix('/api/')
			if B not in Bw or A.headers.get(BH)!=D:A._send(403,b'forbidden',H);return
			try:C=Y(A.headers.get(An,0));E=L.loads(A.rfile.read(C)or b'[]');F=Aa(api,B)(*E);A._send(200,L.dumps(F,ensure_ascii=M).encode(U),'application/json; charset=utf-8')
			except G as I:A._send(500,L.dumps({'error':f(I)}).encode(U),AI)
	F=BK(('127.0.0.1',0),J);i.Thread(target=F.serve_forever,daemon=B).start();return F,f"http://127.0.0.1:{F.server_address[1]}/",D
def By():
	H=Bu();I,E,J=Bx(H);K=[J];D=As.create_window(BL,url=E,width=980,height=720,min_size=(700,520),background_color='#12121a')
	if A.environ.get('PM_SELFTEST'):
		import time as F
		def C(*A):C=' '.join(f(A)for A in A);print(C.encode('ascii',Af).decode(),flush=B)
		def L():
			F.sleep(4)
			try:import urllib.request as B;H=B.Request(E+'api/poll',data=b'[]',method='POST');H.add_header(BH,K[0]);I=B.urlopen(H,timeout=5).read()[:80];C('SELFTEST urllib POST:',I)
			...
			D.destroy()
		i.Thread(target=L,daemon=B).start()
	try:As.start(gui='edgechromium')
	finally:I.shutdown()
